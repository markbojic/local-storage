...
public class DirectoryLocalImplementation implements DirectoryManipulation {
			...
			if (selectedPath == null || selectedPath.equals("") || destinationPath == null
				...
				if (Files.exists(oldPath) && Files.exists(newPath)
					...
					try {
						...
						FileUtils.copyDirectory(oldPath.toFile(), Paths.get(newPath + File.separator + name).toFile()); // Copy
																														// directory
																														// with
																														// contents
	 ...
	 * 
	 * @param selectedPath    Path of the chosen directory
	 * @param destinationPath Path on the storage where zipped directory will be
	 *                        uploaded to
	 * @param user            Current user
	 */
	...
	public void listFiles(String path, String extension) {
		if (path == null || path.equals("")) {
			...
			if (extension.equals("all")) {
				...
				if (empty)
					System.out.println("Directory " + dir.getName() + " does not contain any files.");
			}
			// List files with given extension
				...
				if (empty)
					System.out.println(
							"Directory " + dir.getName() + " does not contain any files with given extension.");
			}
	...
	public void listDirectories(String path) {
		if (path == null || path.equals("")) {
			...
			if (empty)
				System.out.println("Directory " + dir.getName() + " does not contain any subdirectory.");
		}
	}
	...
	public void initStorage(String storagePath, AbstractUser user) {
		if (!Files.exists(Paths.get(storagePath.substring(0, storagePath.lastIndexOf(File.separator))))) {
			...
			if (Files.exists(Paths.get(storagePath))) {
				...
				try {
					...
					while (line != null) {
						...
						if (splitter[0].equalsIgnoreCase(user.getUsername())
								&& splitter[1].equals(user.getPassword())) {
				...
				// New Storage
				System.out.println(
						"Create New Storage - " + storagePath.substring(storagePath.lastIndexOf(File.separator) + 1));
	...
	/**
	 * Initializes new storage, creates root directory on given path, sets forbidden
	 * extensions, admin...
	 * 
	 * @param path                Path of the directory that will be used as storage
	 *                            root
	 * @param forbiddenExtensions Array of strings that represents forbidden
